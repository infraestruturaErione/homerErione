      ...
      <section v-if="config.header" class="first-line">
        <div v-cloak class="container">
          ...
          <div class="dashboard-title">
            <a href="#" class="dashboard-logo-link" aria-label="Home">
              <img
                v-if="config.logo"
                :src="config.logo"
                alt="dashboard logo"
            ...
            <h1 v-if="!config.logo">{{ config.title }}</h1>
          </div>
        </div>
      </section>
...
<style scoped>
/* On very large screens allow slightly larger banner */
...
/* Hide compact logo on large screens (avoid duplicate) */
.logo > a > img {
  display: none;
  height: 28px;
}
